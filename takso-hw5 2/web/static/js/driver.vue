...
<div class="well">
  <div class="alert alert-info" v-if="visible">
    ...
    <button class="btn btn-default" id="acceptButton" v-on:click="submitDecision({status: 'accepted'})">Accept</button>
    <button class="btn btn-danger" id="rejectButton" v-on:click="submitDecision({status: 'rejected'})">Reject</button>
  </div>
  ...
  mounted: function() {
    if (auth.socket) {
        ...
        channel.on("requests", payload => {
            this.visible = payload.visibility;
